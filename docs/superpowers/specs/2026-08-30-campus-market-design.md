...
- 使用单轮有限仲裁处理卖家不回应、部分退货退款、可信退回证明，以及三天验收、七天试用和卖家延长质保。
...
- `AFTERSALE_WINDOW`：已确认收货，但仍处于七天试用窗口，可按当前阶段允许的理由发起争议。
- `DISPUTED`：存在活动争议，结算被冻结。
- `REFUNDING_CANCEL`：交付前取消，等待退款结果。
- `CANCELLED`：未支付订单关闭终态。
- `REFUNDED`：订单全额退款终态。
- `SETTLED`：七天试用窗口结束、没有活动普通争议且净额完成结算的终态。
...
`SETTLED`、`CANCELLED` 和 `REFUNDED` 是普通订单状态机的不可逆终态。`SETTLED` 后不再受理普通验收或七天试用争议；仍在订单快照所载卖家延长质保期内的功能故障通过独立质保案件处理，不回退订单状态或撤销既有结算。法律规定不能排除的责任不受平台期限限制。
...
- 验收期：确认收货后 72 小时。
- 试用期：确认收货后 7 天，包含前三天验收期。
- 卖家延长质保：只能承诺 30、90、180 或 365 天，包含七天平台试用期。
...
订单净结算额为实付金额减成功退款金额。只有七天试用窗口结束、没有活动普通争议、没有未决退款或对账异常时才能创建结算记录；卖家延长质保不会继续冻结本次结算。
...
卖家确认交付后，买家可在 48 小时内确认收货；期限届满且没有活动争议时系统自动确认。买家确认或系统自动确认的数据库时间记为 `T0`。`T0` 后前三天为验收期，`T0` 后七天为试用期且包含验收期；七天试用期结束且没有未决事项才进入 `SETTLED`。

### 8.2 争议范围

买家可在 `AWAITING_RECEIPT` 或 `AFTERSALE_WINDOW` 发起争议。`T0` 起 72 小时内可就数量、型号、外观、缺件、描述不符或功能故障提出验收争议；第 4 天至第 7 天只受理交付时已潜伏的非人为功能故障，不受理正常验收即可发现的外观、数量或型号问题。一个订单只包含一个发布项但可购买多件；争议指定正数 `disputedQuantity`，同一订单累计已裁决数量不得超过购买数量，同一数量不能被重复退款。

进水、摔落、错误供电、擅自拆修、正常耗损以及商品页已明确披露的问题不属于非人为功能故障。买家先提交能够建立初步关联的连续测试、商品标识和故障证据，卖家可以用交付前测试或损坏证据反驳；系统不得仅根据“使用后损坏”自动判定卖家责任，证据冲突进入人工复核。
...
普通验收/试用争议不支持部分比例金额或自由输入退款金额，只按批准数量退款；延长质保的维修补偿是唯一例外，金额必须来自可核验报价或票据并受剩余实付额约束。首版不支持多轮申诉或 AI 自动裁决。
...
商品发布时，卖家可选择不提供额外质保，或选择 30、90、180、365 天的标准期限。期限从 `T0` 起算并包含平台七天试用期。卖家质保采用平台固定的“非人为功能故障”范围；自由文本只能补充说明，不能缩短固定范围、排除故意隐瞒或改变期限。厂家质保使用独立字段保存凭证与到期日，不得冒充卖家质保；厂家凭证无效或与发布承诺不符可作为描述不实证据。

所有窗口采用左闭右开边界：`databaseNow < deadline` 时允许提交，到达截止时刻即过期。前三天验收理由在 `T0 + 72h` 精确切换为仅功能故障，平台试用在 `T0 + 7d` 精确结束，延长质保在订单快照对应截止时刻精确结束。

商品发布后可以修改后续订单的质保选项，但每个订单必须保存下单时的质保类型、期限、范围和厂家凭证摘要快照。买家在第七天后、订单快照的卖家质保截止时刻前提出故障时，创建独立 `warranty_case`，不把 `SETTLED` 订单重新置为 `DISPUTED`。卖家有 72 小时响应；协商失败沿用单轮管理员复核、7 天 SLA、14 天硬期限和证据访问规则。

质保裁定只能选择驳回、维修费用补偿、部分退款或退货退款。维修补偿以可核验报价或票据为依据，采用人民币整数分，批准额不得超过对应商品剩余未退实付额；任一质保案件的赔付、普通退款和历史赔付累计不得超过订单实付金额。退货退款仍须满足可信退回证明，退回商品仍进入隔离库存。

七天后订单正常结算，不因 30 至 365 天质保长期冻结卖家资金。质保责任成立后创建 `seller_obligation`，卖家应在 72 小时内提供资金；可以主动清偿，也可以从未来可结算款中按幂等业务键抵扣。资金落实后才申请面向买家的退款或补偿，平台不得以自有资金垫付。逾期未足额清偿时禁止卖家发布新商品和提现；清偿完成后幂等解除限制。义务状态、抵扣、退款和限制变化全部记录审计与 Outbox。
...
- `listing`：卖家、标题、描述、分类、单价分、可售/隔离数量、卖家质保标准期限、厂家质保凭证摘要/到期日、状态和版本。
...
- `trade_order`：随机订单 ID、买卖双方、商品与质保承诺快照、单价分、数量、总额分、`T0`、验收/试用/延长质保截止时间、状态和版本。
...
- `dispute_evidence`：可关联普通争议或质保案件的逻辑证据 ID、案件类型、提交者、真实类型、大小、随机 Object Key 和时间。
- `return_case`：退回状态、可信证明类型、确认 actor、批准数量和期限。
- `warranty_case`：订单质保快照、故障数量、证据、状态、卖家/管理员期限、裁定、赔付金额分和版本；订单保持终态。
- `seller_obligation`：质保案件、应履行/已筹资金金额分、资金截止时间、未来结算抵扣、限制状态、幂等业务键和版本。
...
对象上传先在短事务中创建 `object_upload_session`，再在事务外流式写入 MinIO，最后在短事务中绑定 `listing_media` 或普通争议/质保案件的 `dispute_evidence` 并完成会话。绑定失败、客户端断开或即时删除失败时创建或保留 `storage_cleanup_task`；清理器在事务外删除对象，并以 claim token 条件完成任务。对象不存在视为幂等成功，数据库事务期间禁止执行 MinIO IO。
...
所有期限任务从数据库截止时间领取，不依赖进程内定时器或 RabbitMQ TTL 作为事实源。调度器包括支付超时、卖家交付超时、买家确认超时、七天试用窗口关闭、卖家退回确认超时、质保卖家响应/筹资超时、管理员 SLA/硬期限、支付退款对账和搜索同步。
...
POST /api/orders/{orderId}/warranty-cases
POST /api/warranty-cases/{caseId}/responses
POST /api/warranty-cases/{caseId}/evidence
GET  /api/warranty-cases/{caseId}/evidence/{evidenceId}/content
POST /api/admin/warranty-cases/{caseId}/decisions
GET  /api/seller-obligations
POST /api/seller-obligations/{obligationId}/fund
POST /api/orders/{orderId}/reviews
```

`POST /api/orders`、取消、支付创建、交付、确认收货、普通/质保案件创建与裁决、退回确认、卖家义务筹资和退款相关命令必须使用 `Idempotency-Key`。同一键和相同请求返回原始 UTF-8 终态响应；同一键配不同请求摘要返回 409。失败事务不遗留阻塞性幂等记录。
...
- 三天验收与七天试用的原因边界、30/90/180/365 天质保期限和订单快照。
- 质保赔付累计上限、卖家义务、发布/提现限制及清偿后解除。
...
- 七天结算边界与质保申请并发时只有一个合法结果，已结算订单不回退。
- 质保义务主动清偿和未来结算抵扣不重复，未清偿限制与清偿解除保持一致。
...
- 退款预占、成功、失败和最老未决退款延迟。
- 普通争议/质保案件状态、卖家响应或筹资超时、未清偿义务、受限卖家、管理员 SLA/硬期限和 `ESCALATED` 数量。
...
经后续评估，上述聊天、竞价、跑腿/代取和真实支付能力涉及通信内容与个人信息处理、交易平台责任、服务规则、支付资质及资金安全等法律与合规问题，当前暂不开展。实验八和实验九仍按各自独立阶段管理，不属于实验七扩展。

已有扩展设计和合规检查材料仅作为历史决策记录保留，不进入实现、测试或上线流程，也不作为实验七验收前置条件。若未来重新启动任何扩展，必须先完成独立法律合规评估并获得明确授权。
