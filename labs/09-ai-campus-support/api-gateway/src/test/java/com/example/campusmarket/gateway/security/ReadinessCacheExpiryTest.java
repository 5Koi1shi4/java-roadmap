...
/** 缓存和注册信息失效后探针必须撤销就绪；短暂中断仍可使用已预热信息。 */
class ReadinessCacheExpiryTest {
    @Test
    void jwksOutageStopsBeingReadyAfterBoundedStaleWindow() {
        MutableClock clock = new MutableClock();
        AtomicReference<String> response = new AtomicReference<>(validJwks());
        WebClient client = WebClient.builder().exchangeFunction(request ->
            response.get() == null ? Mono.error(new IllegalStateException("identity unavailable"))
                : Mono.just(ClientResponse.create(HttpStatus.OK).body(response.get()).build()))
            .build();
        JwksReadinessHealthIndicator indicator = new JwksReadinessHealthIndicator(
            client, "http://identity.test/jwks", clock, Duration.ofSeconds(30));
    ...
    @Test
    void jwksWithoutUsableRs256PublicKeyCannotPrimeReadiness() {
        WebClient client = WebClient.builder().exchangeFunction(request ->
            Mono.just(ClientResponse.create(HttpStatus.OK)
                .body("{\"keys\":[{\"kty\":\"oct\",\"k\":\"YWJj\",\"kid\":\"symmetric\"}]}")
                .build())).build();
        JwksReadinessHealthIndicator indicator = new JwksReadinessHealthIndicator(
            client, "http://identity.test/jwks");

        assertThat(indicator.health().block().getStatus()).isEqualTo(Status.DOWN);
    }

    @Test
    void missingInstanceOrOldRegistryFetchStopsEurekaReadiness() {
        ...
        when(discovery.getInstances("legacy-market-service")).thenReturn(List.of(instance));
        ...
        when(discovery.getInstances("legacy-market-service")).thenReturn(List.of());
        assertThat(indicator.health().block().getStatus()).isEqualTo(Status.DOWN);
    }

    private static String validJwks() {
        return "{\"keys\":[" + TestJwtFactory.publicJwk("test-key-1").toJSONString() + "]}";
    }
