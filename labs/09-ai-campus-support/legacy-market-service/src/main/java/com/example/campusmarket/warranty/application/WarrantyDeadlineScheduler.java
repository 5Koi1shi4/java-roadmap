...
public final class WarrantyDeadlineScheduler {
    ...
    public WarrantyDeadlineScheduler(JdbcTemplate jdbc,org.springframework.transaction.PlatformTransactionManager manager){this(jdbc,manager,null);}
    @Autowired public WarrantyDeadlineScheduler(JdbcTemplate jdbc,org.springframework.transaction.PlatformTransactionManager manager, CampusMetrics metrics){this.jdbc=Objects.requireNonNull(jdbc);this.tx=new TransactionTemplate(Objects.requireNonNull(manager));this.metrics=metrics;}
    ...
    private void expireObligations(){
        List<UUID> ids=jdbc.query("SELECT id FROM seller_obligation WHERE status IN ('AWAITING_FUNDING','PARTIALLY_FUNDED') AND funded_amount_fen<obligation_amount_fen AND funding_deadline<=CURRENT_TIMESTAMP(6)",(rs,n)->UUID.fromString(rs.getString(1)));
    ...
    private boolean process(UUID id,String token){try{return Boolean.TRUE.equals(tx.execute(s->{
        ...
        if(row==null)return complete(id,token);if("SELLER_RESPONSE".equals(c.type())){if(now.isBefore(row.seller()))return defer(id,token,row.seller());if("OPEN".equals(row.status())){Instant admin=now.plus(java.time.Duration.ofDays(7)),hard=now.plus(java.time.Duration.ofDays(14));int changed=jdbc.update("UPDATE warranty_case SET status='UNDER_REVIEW',admin_deadline=?,hard_deadline=?,version=version+1,updated_at=CURRENT_TIMESTAMP(6) WHERE id=? AND status='OPEN'",Timestamp.from(admin),Timestamp.from(hard),c.caseId().toString());if(changed==1&&metrics!=null)metrics.recordSellerResponseTimeout();arm(c.caseId(),admin,hard);}}else if("ADMIN_SLA".equals(c.type())){if(row.admin()==null||now.isBefore(row.admin()))return defer(id,token,row.admin()==null?now.plusSeconds(1):row.admin());int changed=jdbc.update("UPDATE warranty_case SET status=CASE WHEN status IN ('OPEN','SELLER_RESPONDED','UNDER_REVIEW') THEN 'ESCALATED' ELSE status END,admin_sla_alerted_at=CURRENT_TIMESTAMP(6),updated_at=CURRENT_TIMESTAMP(6) WHERE id=? AND status IN ('OPEN','SELLER_RESPONDED','UNDER_REVIEW','ESCALATED') AND admin_sla_alerted_at IS NULL",c.caseId().toString());if(changed==1&&metrics!=null)metrics.recordAdminSlaTimeout();}else if("HARD_DEADLINE".equals(c.type())){if(row.hard()==null||now.isBefore(row.hard()))return defer(id,token,row.hard()==null?now.plusSeconds(1):row.hard());int changed=jdbc.update("UPDATE warranty_case SET status='ESCALATED',updated_at=CURRENT_TIMESTAMP(6) WHERE id=? AND status IN ('OPEN','SELLER_RESPONDED','UNDER_REVIEW')",c.caseId().toString());if(changed==1&&metrics!=null)metrics.recordHardDeadlineEscalation();}return complete(id,token);}));}catch(RuntimeException e){jdbc.update("UPDATE warranty_deadline_claim SET status='NEW',owner_id=NULL,claim_token=NULL,lease_until=NULL,updated_at=CURRENT_TIMESTAMP(6) WHERE id=? AND owner_id=? AND claim_token=? AND lease_until>CURRENT_TIMESTAMP(6)",id.toString(),owner,token);throw e;}}
    ...
    private void recordTransition(UUID seller,String action,UUID obligation,long version,String payload){
        if (metrics != null) metrics.recordFundingTimeout();
