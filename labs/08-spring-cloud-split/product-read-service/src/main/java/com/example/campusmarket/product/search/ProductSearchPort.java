...
public interface ProductSearchPort {
    ...
    default void indexInto(String index, ProductDocument document) {
        index(document);
    }
    ...
    /** Raised when the read-side search dependency cannot serve a request. */
    ...
    record ProductDocument(String listingId, String title, String description, String category,
        ...
        public ProductDocument {
            ...
            if (title == null || (title.isBlank() && !"TOMBSTONE".equals(status))) {
                throw new IllegalArgumentException("标题不能为空");
            }
            if (description == null || (description.isBlank() && !"TOMBSTONE".equals(status))) {
                throw new IllegalArgumentException("描述不能为空");
            }
            if (category == null || (category.isBlank() && !"TOMBSTONE".equals(status))) {
        ...
        public static ProductDocument tombstone(String listingId, long version) {
            return new ProductDocument(listingId, "", "", "", 0, 0, "TOMBSTONE", version);
        }
    }
    ...
    record SearchRequest(String keyword, String category, Long minPriceFen, Long maxPriceFen,
        ...
        public SearchRequest(String keyword, String category, Long minPriceFen, Long maxPriceFen,
                             int page, int size) {
        ...
        public SearchRequest {
            ...
            if (page != 0 || size <= 0 || size > 100) {
                throw new IllegalArgumentException("请使用 search_after 游标分页");
            }
    ...
    record SearchCursor(String pitId, String fingerprint, double score, String listingId) {
        public SearchCursor(double score, String listingId) {
            this("", "", score, listingId);
        }

        public SearchCursor {
            if (pitId == null || fingerprint == null || pitId.length() > 500 || fingerprint.length() > 200
                    || !Double.isFinite(score) || listingId == null || listingId.isBlank()
                    || listingId.length() > 200 || listingId.indexOf('\n') >= 0 || listingId.indexOf('\r') >= 0) {
    ...
    static SearchCursor decodeCursor(String encoded) {
        ...
        try {
            ...
            if (parts.length == 2) {
                ...
                if (!Double.toHexString(score).equals(parts[0])) {
                    throw new IllegalArgumentException("游标 score 非 canonical");
                }
            ...
            if (!Double.toHexString(score).equals(parts[2])) {
                throw new IllegalArgumentException("游标 score 非 canonical");
            }
