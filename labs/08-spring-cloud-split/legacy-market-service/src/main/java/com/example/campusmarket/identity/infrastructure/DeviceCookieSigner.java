...
public class DeviceCookieSigner {
    ...
    public DeviceCookieSigner(@Value("${campus.market.identity.verification-secret}") String secret) {
        if (secret == null || secret.getBytes(StandardCharsets.UTF_8).length < 32) {
            throw new IllegalArgumentException("campus verification secret must be at least 32 bytes");
        }
    ...
    private String signature(String payload) {
        try {
            ...
            throw new IllegalStateException("Unable to sign device cookie", ex);
        }
    }
}
