...
public final class WarrantyController {
    ...
    public ResponseEntity<byte[]> decide(@PathVariable UUID caseId,@RequestBody DecisionRequest req,@RequestHeader(value="Idempotency-Key",required=false) String idempotencyKey,Authentication auth){try{if(!admin(auth))return error(HttpStatus.FORBIDDEN,"无权执行该角色操作");String key=idempotencyKey==null||idempotencyKey.isBlank()?"legacy-"+caseId+"-"+req.decision():idempotencyKey;WarrantyService.Result r=warranties.decide(caseId,user(auth),req.decision(),req.compensationAmountFen(),req.evidenceId(),key);return ResponseEntity.ok().contentType(JSON).body(("{\"caseId\":\""+r.caseId()+"\",\"status\":\""+r.status()+"\"}").getBytes(StandardCharsets.UTF_8));}catch(WarrantyService.NotFoundException e){return error(HttpStatus.NOT_FOUND,"质保案件不存在");}catch(WarrantyService.IdempotencyConflictException e){return error(HttpStatus.CONFLICT,"幂等冲突");}catch(IllegalArgumentException|IllegalStateException e){return error(HttpStatus.CONFLICT,"质保裁定无效");}}
