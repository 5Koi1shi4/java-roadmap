...
- 验证：实验代码只保存在独立分支 `learning/campus-market`，不合并到 `main`。JDK 17 下 `mvnw.cmd test` 为 137 项；MySQL 8.4、Redis 7.4、RabbitMQ 3.13、SmartCN Elasticsearch 8.18.8、MinIO 与 Toxiproxy 环境中的完整 `mvnw.cmd verify` 为 251 个 Failsafe/Testcontainers 集成测试；均为 0 failures、0 errors、0 skipped（实验文档验收提交 `85e1d1f`）。
- 边界：正式 CAS、真实支付和真实物流没有接入。设计中已有 `7.1` 聊天、`7.2` 竞价、`7.3` 跑腿/代取和真实支付适配器的扩展路线，但当前没有独立分支、目录、设计、计划或验收记录，状态为“仅规划未启动”。
